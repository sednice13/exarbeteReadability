...
function createDoc(options) {
  ...
  if (app.options.markdown === true) {
    markdownParser = new Markdown({
      ...
      linkify: false,
      typographer: false,
      highlight: (str, lang) => lang ? `<pre class="prettyprint lang-${lang}">${str}</pre>` : `<pre class="prettyprint">${str}</pre>`
    });
  } else if (app.options.markdown !== false) {
    if (!path.isAbsolute(app.options.markdown) && !app.options.markdown.startsWith('~')) {
  ...
  try {
    ...
    const json = JSON.parse(fs.readFileSync(path.join(apidocPath, 'package.json'), 'utf8'));
...
function createOutputFiles(api) {
  ...
  if (!app.options.simulate) fs.writeFileSync(`${app.options.dest}./api_data.js`, `define({ "api": ${api.data} });\n`);

  // Write api_project.json`);
  ...
  if (!app.options.simulate) fs.writeFileSync(`${app.options.dest}./api_project.js`, `define(${api.project});\n`);
  ...
  if (!app.options.simulate && !app.options.copyDefinitions) fs.writeFileSync(`${app.options.dest}./api_definition.js`, `define({ "api": ${api.definitions} });\n`);
}
